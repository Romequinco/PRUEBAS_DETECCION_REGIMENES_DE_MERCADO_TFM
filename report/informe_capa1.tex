...
\begin{description}[leftmargin=0pt,labelsep=0.5em,itemsep=2pt]
  ...
  \item[D10 -- turbulencia de Mahalanobis.] \cite{kritzman2012} Distancia multivariante con covarianza
...
\begin{table}[H]
  ...
  \code{results/metrics_master_final.csv}). \emph{switching}: conmutaciones por d\'ia
...
\begin{description}[leftmargin=1.2em,style=nextline]
  ...
  crisis sist\'emicas (2008, 2011, 2020, 2022) y dos ventanas-trampa (2013, 2018). \textbf{No
  hay tests de significancia ni intervalos de confianza}; diferencias de pocos puntos entre
  detectores \emph{no} son estad\'isticamente distinguibles. El podio es evidencia
...
(\emph{stationary/circular block bootstrap}) sobre las series OOS, para obtener bandas de
confianza de cobertura, falsa alarma y conmutaci\'on, y contrastar pares de detectores
respetando la autocorrelaci\'on. (B2)~\textbf{An\'alisis de sensibilidad de
...
Todos los n\'umeros de este informe provienen \emph{verbatim} de
\code{results/metrics_master_final.csv} (con el BIC de D4 corroborado en
\code{results/metrics_04_hmm_gaussian_2s.csv}). Las figuras \code{pdf_*.png} se regeneran de
forma determinista desde ese CSV con \code{python scripts/figs_pdf/build_pdf_figs.py}; las
figuras \code{eda_*.png} provienen del an\'alisis exploratorio. No se ha re-ejecutado
ning\'un detector ni recalculado ninguna m\'etrica para producir este documento.
